...
export const createLinkedInProfile = async (req, res) => {
  ...
  try {
    ...
    const { firstName, lastName, profressionalHeadline, university, keySkills, objective } =
      req.body;
...
export const getLinkedInProfile = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "Linked In Profile Retrieved Succesfully!",
      linkedInProfile: linkedInProfile
    });
  } catch (error) {
...
export const updateLinkedInProfile = async (req, res) => {
  ...
  try {
    ...
    linkedInProfile.profressionalHeadline =
      profressionalHeadline ?? linkedInProfile.profressionalHeadline;
    ...
    for (let i = 0; i < skillKeys.length; i++) {
      ...
      if (linkedInProfile.keySkills.hasOwnProperty(skill)) {
        linkedInProfile.keySkills[skill] =
          req.body.keySkills[skill] ?? linkedInProfile.keySkills[skill];
      }
    }

    await linkedInProfile.save();
    return res.status(201).json({
      message: "Linked In profile updated succesfully!",
      linkedInProfile: linkedInProfile
    });
  } catch (error) {
    return res.status(500).json({ message: "Server error", error: error.message });
...
export const getEventsToAttend = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Events found succesfully!", eventsToAttend: eventsToAttend });
  } catch (error) {
    return res.status(500).json({ message: "Server error", error: error.message });
  }
};
...
export const updateEventsToAttend = async (req, res) => {
  ...
  try {
    ...
    attendingEventIds.forEach((updateEvent) => {
      const idx = userEvents.attendingEventIds.findIndex((e) => e.eventId === updateEvent.eventId); // Checking if an event exists in the database for the user (hence idx == -1 would mean that it is not in the user's events saved in the database)
...
export const createReflection = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ message: "Reflection saved!", reflection: newNetworkingReflection });
  } catch (error) {
    return res.status(500).json({ message: "Server error", error: error.message });
  }
};
...
export const getReflections = async (req, res) => {
  ...
  try {
    const reflections = await networkingReflection.find({ user: userId }).lean();
    const reflectionsWithEvents = reflections.map((r) => ({
      ...r,
      event: allEvents.find((e) => e.id === r.event) || null
    }));
    return res
      .status(200)
      .json({ message: "Reflections found succesfully!", reflections: reflectionsWithEvents });
  } catch (error) {
    return res.status(500).json({ message: "Server error", error: error.message });
  }
};
...
export const getLinkedInPost = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "LinkedIn post found succesfully!", linkedInPost: linkedInPost });
  } catch (error) {
    return res.status(500).json({ message: "Server error", error: error.message });
  }
};
...
export const getAllEvents = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: "All events retrieved succesfully!",
      allEventsFromBackend: allEventsFromBackend
    });
  } catch (error) {
    return res.status(500).json({ message: "Server error", error: error.message });
  }
